...
class KlgHttpServerPatcher extends pandora_hook_1.HttpServerPatcher {
    constructor(options) {
        super(options);
    }
    /**
    ...
    recordResponse(options, req, res, tracer, span) {
        ...
        function handleResponse(span, chunk) {
            const response = Utils_1.safeParse(chunk);
            span.log({
                response
            });
        }
    ...
    bufferTransformer(buffer) {
        const dataStr = buffer.toString('utf8');
        let data = Utils_1.safeParse(dataStr);
        if (typeof (data) === 'string') {
            data = querystring_1.parse(dataStr);
        }
        try {
            return data;
        }
    ...
    recordPostData(options, req, res, tracer, span) {
        ...
        if (options.recordPostData && req.method && req.method.toUpperCase() === 'POST') {
            shimmer.wrap(req, 'emit', function wrapRequestEmit(emit) {
                ...
                return function wrappedRequestEmit(event) {
                    if (event === 'data') {
...
exports.KlgHttpServerPatcher = KlgHttpServerPatcher;
